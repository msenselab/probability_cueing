...
try
    ...
    circle_diam = 1.5;
    box_size = 1.5*sqrt(pi/4);
    % create stimuli (Square-Green, Sq-Red, Circle-Green, Circle-red)
    container(1) = v.createShape('rectangle', box_size, box_size, 'fill',0,'color',[0,196,0]);  % [50,172,34] after moniter changing
    container(2) = v.createShape('rectangle', box_size, box_size,'fill',0, 'color',[196,0,0]);  % [250,51,72] after moniter changing
...
%    rects = ones(8,2)*box_size;     % sizes of each container
    ...
    num_items = [4 8 12]; % number of items at each ring
    tot_items=sum(num_items);
    icount = 1;
    theta0 = [0 22.5 0]/180*pi;
    eccs = [1.8 4 6.5];
    for i=1:length(num_items)
        ecc = eccs(i); % 1.5 degree
        for k = 1:num_items(i)
            theta = theta0(i) + 2*pi/num_items(i)*(k-1);
            xy(icount, :) = ecc*[cos(theta), sin(theta)];
            icount = icount + 1;            
        end
    end
    rects = ones(tot_items,2)*box_size;     % sizes of each container
    
    
    % start the experiment
    v.dispText(['Visual Search \n', ...
        'In this experiment, you will see a display with many objects (circle/diamond). And one of the shapes is different from the other seven. \n', ...
    ...
    for iTrl=1: exp.maxTrls
        ...
        tarPos = cond(1) + 4; % target location
        ...
        disPos(disPos>0)=disPos(disPos>0)+4;
      
        
        % initialize all distractors
        if tarContainer == 1 % Diamond
            if distColor == 1 
                boxes = repmat(container(3), 1,tot_items); % distractor as circle
            else
                boxes = repmat(container(4), 1,tot_items); % distractor as circle
            end
           ...
           if distColor == 1 
               boxes = repmat(container(1), 1,tot_items); % distractors as diamond
           else
               boxes = repmat(container(2), 1,tot_items); % distractors as diamond
           end
        ...
        line_ori = repmat([1,2], 1,tot_items/2);
        ...
        l_rects = repmat([line_length,line_thick], tot_items,1);
        for ipos = 1:tot_items
        ...
        rotations = repmat(45,1,tot_items); % change square to diamond
        
        % change sizes to match requirements of the shapes on this trial
        if tarContainer==2
            rects=ones(tot_items,2)*box_size;
            rects(tarPos,:)=circle_diam;
        else
            rects=ones(tot_items,2)*circle_diam;
